...
const getDashboard = async (req, res) => {
  try {
    const cafeteria = await pool.query(
      'SELECT id, nombre FROM cafeterias WHERE gerente_id = $1 LIMIT 1',
      [req.usuario.id]
    );
...
const crearBarista = async (req, res) => {
  ...
  try {
    ...
    const rol = await pool.query(
      'SELECT id FROM roles WHERE nombre = $1', ['barista']
    );
    ...
    res.status(201).json({
      message: `Barista ${nombre} creado exitosamente`,
      barista: usuario.rows[0]
    });
  } catch (err) {
    console.error(err);
...
const getPedidosHoy = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT p.id, p.mesa, p.estado, p.creado_en,
              p.notas_cliente,
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT c.id, c.variedad, c.proceso, c.qr_codigo,
       ...
       LEFT JOIN valoraciones v ON v.pedido_id   = p.id
       WHERE cc.cafeteria_id = $1 AND cc.activa = true
       GROUP BY c.id, f.nombre, f.municipio, f.altitud_msnm,
                u.nombre, cc.asignado_en
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT DISTINCT
       ...
       LEFT JOIN valoraciones v    ON v.pedido_id = p.id
       WHERE r.nombre = 'barista' AND u.activo = true
       GROUP BY u.id, ca.id, ca.nombre, t.id, t.nombre,
                t.estado, t.hora_inicio, t.hora_fin, t.fecha
...
const cambiarEstadoTurno = async (req, res) => {
  ...
  try {
    ...
    // Si se activa un turno cerrar los demás activos
    if (estado === 'activo') {
      await pool.query(
        `UPDATE turnos SET estado = 'cerrado'
         WHERE cafeteria_id = $1 AND estado = 'activo' AND id != $2`,
...
const eliminarTurno = async (req, res) => {
  try {
    ...
    if (parseInt(pedidos.rows[0].count) > 0) {
      return res.status(400).json({
        error: 'No se puede eliminar un turno que tiene pedidos registrados'
      });
    }

    // Eliminar baristas asignados primero
    await pool.query(
      'DELETE FROM turno_baristas WHERE turno_id = $1',
      [req.params.id]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT
         ...
         COALESCE(SUM(pa.monto) FILTER
           (WHERE pa.estado='confirmado'),0)             AS ingresos,
         ROUND(AVG(v.cafe_experiencia)::numeric,1)      AS satisfaccion
       FROM pedidos p
       LEFT JOIN pagos pa       ON pa.pedido_id = p.id
       LEFT JOIN valoraciones v ON v.pedido_id  = p.id
...
// module.exports — añade getVentas
module.exports = {
